...
Future<Status> checkUpdates() async {
  ...
  try {
    ...
    if (assetTokenVersion > cacheTokenVersion ||
        !storage.hasData(itemsContentKey) ||
        !storage.hasData(tokenKey)) {
      storage.write(tokenVersionKey, assetTokenVersion);
      storage.write(tokenKey, metadata[tokenKey]);
      storage.write(
          itemsContentKey,
          await rootBundle.loadString('assets/json/map_items.json',
              cache: false));
    }
    ...
    developer.log('Info: czas pobierania aktualizacji z firebase: ' +
